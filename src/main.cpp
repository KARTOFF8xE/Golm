...
void loop() {
    ...
    if (request.indexOf("GET /setnum") != -1) {
        ...
        client.println("Content-Type: text/plain");
        client.println();
        client.println(String(storedNumber));
        client.stop();
        return;
    }
    ...
    client.println("<p>Current Vel: <strong id='numberDisplay'>" + String(storedNumber) + "</strong></p>");
    client.println("<input type='number' id='numberInput' value='" + String(storedNumber) + "'>");
